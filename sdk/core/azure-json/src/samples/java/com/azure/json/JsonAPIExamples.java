...
public class JsonAPIExamples {
    ...
    public static void main(String[] args) throws IOException {
        ...
        JsonElement bookLog = ((JsonObject)library).getProperty("books");
        JsonElement childLibrary = new JsonArray();
        for (int i = 0; i < ((JsonArray)bookLog).size(); i++){
            JsonBoolean getCheck = (JsonBoolean) ((JsonObject)((JsonArray)bookLog).getElement(i)).getProperty("child friendly");
            if (getCheck.getValue()){
                ((JsonArray)childLibrary).addElement(((JsonArray)bookLog).getElement(i));
                ((JsonArray)bookLog).removeElement(i);
            }
        }
